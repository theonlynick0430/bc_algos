...
def make_policy(policy_class, policy_config, config):
    if policy_class == 'ACT':
        policy = ACTPolicy(policy_config, config)
...
def load_act_policy(core_config, config):
    num_epochs = core_config['num_epochs']
    ckpt_dir = core_config['ckpt_dir']
    seed = core_config['seed']
    policy_class = core_config['policy_class']
    policy_config = core_config['policy_config']

    policy = make_policy(policy_class, policy_config, config)
    policy.load_state_dict(torch.load("/home/niksrid/bc_algos/single_goals_subgoal1_type_token_difficult_gen/policy_epoch_800_seed_0.ckpt"))
    policy.eval()
    policy.cuda()
    return policy

def test(config, config_act, core_config_act):
    ...
    policy = load_act_policy(core_config=core_config_act, config=config_act)
    ...
    with tqdm(total=validset.num_demos, unit='demo') as progress:
        for demo_id in validset.demos:
            ...
            print("pick status:", rollout_info["success"]["pick_success"])
            print("place status:", rollout_info["success"]["put_success"])
            progress.update(1)
    # deinit obs utils
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/home/niksrid/bc_algos/datasets/preprocessed/dataset_v3_test_16hz"
    )
    ...
    parser.add_argument(
        ...
        default="/home/niksrid/bc_algos_act/policy_epoch_1995_seed_0.ckpt"
    )
    ...
    parser.add_argument(
        ...
        default="/home/niksrid/bc_algos/debug"
    )
    ...
    core_config_act = {
        'num_epochs': config_act['num_epochs'],
        'ckpt_dir': config_act['ckpt_dir'],
        # 'episode_len': episode_len,
        'state_dim': 14, #hardcode
        'lr': config_act['lr'],
        'policy_class': config_act['policy_class'],
        'onscreen_render': config_act['onscreen_render'],
        'policy_config': get_act_policy_config(config=config_act),
        # 'task_name': task_name,
        'seed': config_act['seed'],
        'temporal_agg': config_act['temporal_agg'],
        # 'camera_names': camera_names,
        'real_robot': not config_act['is_sim']
    }
    test(config=config, config_act=config_act, core_config_act=core_config_act)